...
def run(
    ...
    for url, local_file in inputs:
        ...
        if generate_timeline:
            ...
            write_timeline_outputs(video_id, url, windows, reports_dir, keyframes_dir)
            print(
        ...
        if run_analysis and not ollama_available:
            ...
            try:
                raw = analyze_transcript_with_ollama(full_transcript, model, url)
            except RuntimeError as exc:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        default="llama3.1",
        help="Ollama model name for transcript analysis (default: llama3.1).",
    )
    parser.add_argument(
        "--vision-model",
        default="llama3.2-vision",
        help="Ollama vision model for keyframe+transcript summary (default: llama3.2-vision).",
    )
    parser.add_argument(
        "--work-dir",
        default="artifacts",
    ...
    parser.add_argument(
        ...
        default="none",
        help="Vision backend for per-frame OCR text extraction (default: none).",
    )
    parser.add_argument(
        "--keyframe-mode",
